...
(function(){
    ...
    const mapContainer = $('#sitterMapContainer');
    ...
    function initSitterMap(shouldRecenter = false) {
        ...
        if (!sitterMap) {
            ...
            sitterMap = L.map('sitterMapContainer').setView([centerLat, centerLng], 13);
        ...
        useMyLocationBtn.addEventListener('click', (e) => {
            ...
            navigator.geolocation.getCurrentPosition(
                async (position) => {
                    ...
                    sitterMap.setView([latitude, longitude], 15);
                    
                    latInput.value = latitude.toFixed(6);
                    lngInput.value = longitude.toFixed(6);

                    if (sitterMarker) {
                        sitterMarker.setLatLng([latitude, longitude]);
                    } else {
                        sitterMarker = L.marker([latitude, longitude]).addTo(sitterMap);
                    }
        ...
        try {
            ...
            if (data.success) {
                ...
                if (data.sitter) {
                    ...
                    if (data.sitter.location_latitude) {
                        latInput.value = data.sitter.location_latitude;
                    }
                    if (data.sitter.location_longitude) {
                        lngInput.value = data.sitter.location_longitude;
                    }
                    if (data.sitter.location_latitude && data.sitter.location_longitude) {
                        updateLocationDisplay(parseFloat(data.sitter.location_latitude), parseFloat(data.sitter.location_longitude));
                    }
                }
    ...
    async function saveSettings(formId, dataKey) {
        ...
        if (formId === '#formPassword') {
            ...
            const fd = new FormData();
            fd.append('sitter[work_area]', workArea ? workArea.value.trim() : '');
            ...
            fd.append('sitter[location_longitude]', lngInput ? lngInput.value.trim() : '');
            
            // For sitter form, send FormData instead of JSON
            try {
                const response = await fetch('/PETVET/api/sitter/update-settings.php', {
                    method: 'POST',
                    body: fd
                ...
                if (result.success) {
                    showToast('Sitter settings saved successfully');
                    form.dataset.clean = 'true';
                    updateButtonState(form, submitBtn);
                } else {
                    showToast(result.message || 'Failed to save');
