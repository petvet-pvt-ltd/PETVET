<?php
/**
 * API Endpoint: Approve Vet Request
 * Clinic manager approves a pending vet role request for their clinic.
 */
...
try {
    ...
    if ((int)$row['clinic_id'] !== $clinicId) {
        http_response_code(403);
        echo json_encode(['success' => false, 'error' => 'No permission to approve this request']);
        exit;
    }
    ...
    // Activate vet availability (also ensure not suspended/on-leave)
    $stmt = $pdo->prepare('UPDATE vets SET available = 1, is_suspended = 0, suspended_at = NULL, is_on_leave = 0, updated_at = CURRENT_TIMESTAMP WHERE user_id = ? AND clinic_id = ?');
