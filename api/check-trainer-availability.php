...
try {
    ...
    $stmt = $db->prepare("
        SELECT block_type, block_time, reason
    ...
    if ($blockedDate) {
        if ($blockedDate['block_type'] === 'full-day') {
            echo json_encode([
                'success' => true,
                'available' => false,
                'message' => 'Trainer is unavailable on this date' . ($blockedDate['reason'] ? ': ' . $blockedDate['reason'] : '')
            ]);
            exit;
        } elseif ($blockedDate['block_type'] === 'before' || $blockedDate['block_type'] === 'after') {
            // Handle before/after time blocks (only if time is provided)
            if ($time) {
