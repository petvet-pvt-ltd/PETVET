...
function showForm(prefill=false, apptId=null){
  ...
  if(prefill && apptId){
    ...
    if(appt){ form.elements['appointmentId'].value = appt.id; form.elements['petName'].value = appt.petName; form.elements['ownerName'].value = appt.ownerName; }
...
function renderVaccinations(list){
  ...
  if(!arr || arr.length===0){ c.innerHTML = '<p>No vaccinations.</p>'; return; }
  let html = '<table><thead><tr><th>ID</th><th>Date</th><th>Pet</th><th>Owner</th><th>Vaccine</th><th>Next Due</th></tr></thead><tbody>';
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  if(from==='ongoing' && apptId){ showForm(true, apptId); renderVaccinations(getData().vaccinations.filter(v=>v.appointmentId===apptId)); }
  else if(from==='completed' && apptId){ showForm(false); renderVaccinations(getData().vaccinations.filter(v=>v.appointmentId===apptId)); }
  else { showForm(false); renderVaccinations(); }
  ...
  if(form) form.addEventListener('submit', e=>{
    ...
    form.reset();
    form.elements['appointmentId'].value=n.appointmentId; form.elements['petName'].value=n.petName; form.elements['ownerName'].value=n.ownerName;
  });
});
